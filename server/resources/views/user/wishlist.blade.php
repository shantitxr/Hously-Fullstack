      ...
      <div class="card bg-white shadow-md rounded-2xl overflow-hidden">
        <figure class="relative">
          ...
          <form method="POST" action="{{ route('wishlist.toggle', $property) }}"
            ...
            @csrf
            <button type="submit" class="w-10 h-10 bg-white rounded-full flex items-center justify-center shadow-lg hover:bg-red-50">
        ...
        <div class="card-body p-5">
          ...
          <div class="flex items-center gap-4 text-gray-600 text-sm mt-2">
            ...
            <span>{{ $property->bathrooms }} Baths</span><span>•</span>
          ...
          <div class="flex items-center justify-between mt-4 pt-4 border-t border-accent">
            ...
            <div class="flex gap-2">
              ...
              <form method="POST" action="{{ route('wishlist.toggle', $property) }}">
                @csrf
                <button type="submit" class="btn btn-sm bg-red-100 hover:bg-red-200 text-red-600 border-none rounded-lg">Remove</button>
              </form>
            </div>
          </div>
        </div>
