...
@section('content')
  ...
  <div class="bg-white rounded-2xl p-6 shadow-md mb-8">
    ...
    <div class="flex flex-wrap gap-4">
      <a href="{{ route('user.properties.create') }}" class="btn bg-primary hover:bg-secondary text-white border-none rounded-xl">Add New Property</a>
      <a href="{{ route('user.properties.index') }}" class="btn bg-accent hover:bg-secondary text-dark border-none rounded-xl">View All Properties</a>
      <a href="{{ route('inquiries.index') }}" class="btn bg-accent hover:bg-secondary text-dark border-none rounded-xl">Check Inquiries</a>
    </div>
  </div>
        ...
        @forelse($recentInquiries as $inquiry)
          <div class="flex items-start gap-4 p-4 bg-background rounded-xl">
            <div class="w-10 h-10 bg-accent rounded-full flex items-center justify-center shrink-0">
              {{-- sender is the User who sent the inquiry --}}
              <span class="text-primary font-semibold text-sm">
                {{ strtoupper(substr($inquiry->sender->name ?? '?', 0, 2)) }}
              </span>
            </div>
          ...
          <div class="flex items-center gap-4 p-3 bg-background rounded-xl">
            @if($property->image_path)
              <img src="{{ Storage::url($property->image_path) }}"
                   alt="{{ $property->title }}" class="w-16 h-14 object-cover rounded-lg"/>
            @else
              <div class="w-16 h-14 bg-accent rounded-lg flex items-center justify-center">
            ...
            {{-- is_available is the actual DB field, not is_active --}}
